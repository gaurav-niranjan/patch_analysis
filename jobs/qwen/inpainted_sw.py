...
def run_experiment(sample_id, frac, inpaint_style):
    ...
    if inpaint_style == "original":
        inputs, image_path = get_input(processor, ds, id_to_index, sample_id, IMAGE_DIR, downsample_image,inpaited=False) #Change there to use inpainted
    else:
        inputs, image_path = get_input(processor, ds, id_to_index, sample_id, IMAGE_DIR, downsample_image, inpaited=True, style=inpaint_style) #Change there to use inpainted 
